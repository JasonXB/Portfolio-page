...
import { lightThemeColors, darkThemeColors } from "../../styles/themesMUI";
...
export default function PaddedWavyBlock(props) {
  ...
  const { theme } = useGlobalContext(); // Select an svg fill color based on the active theme
  return (
    <>
      <Box
        sx={(mui) => ({
          ...styles.outerBox,
          display: "block",
          background: mui.palette.wave.main,
        })}
      >
        <Box
      ...
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
        <path
          fill={
            theme === "light"
              ? lightThemeColors.wave.main
              : darkThemeColors.wave.main
          }
          fill-opacity="1"
